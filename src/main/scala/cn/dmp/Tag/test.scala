...
object test {
  def main(args: Array[String]): Unit = {
    ...
    // 过滤数据
    val df2 = df.filter(TagUtils.AnyUserId)
    // 处理用户ID
    import sqlContext.implicits._
    val baseRDD: Dataset[(List[String], Row)] = df2.map(row=>{
      // 获取当前行上的所有的用户非空ID
      val userAllId: List[String] = TagUtils.getAllUserId(row)
      (userAllId,row)
    })
    // 构建点的集合
    val verites: Dataset[(Long, List[(String, Int)])] = baseRDD.flatMap(t=>{
      val row = t._2
      // 把标签处理一下
      // 广告位类型 和 渠道标签
      val adTag: List[(String, Int)] = ADTags.label(row)
      // APP标签
      val appTag = AppTags.label(row,cast)
      // 设备标签
      val deviceTag = DeviceTags.label(row)
      // 关键字标签
      val KwTag = KeywordTags.label(row,stopWordCast)
      // 地域标签
      val LocationTag = AreaTags.label(row)
      // 先把标签加到一起
...
//    // 构造边的集合: RDD[Edge[Int]]
    val edges: Dataset[Edge[Int]] = baseRDD.flatMap(t => {
      // A B C  a->b  a->c
      t._1.map(uid => Edge(t._1.head.hashCode, uid.hashCode.toLong, 0))
    })
    //edges.take(20).foreach(println)
    // 调用图计算  找出其中的顶点，进行连接
    val graph = Graph(verites.rdd,edges.rdd)
    // 取顶点
    val cc = graph.connectedComponents().vertices
    // 聚合数据
    val result: RDD[(VertexId, List[(String, Int)])] = cc.join(verites.rdd).map{
